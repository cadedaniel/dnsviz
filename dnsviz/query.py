...
class DNSResponseHandlerFactory(object):
    ...
    def build(self):
        ...
        obj = self._cls.__new__(self._cls, *self._args,
                                __instantiate=True, **self._kwargs)
...
class UseTCPOnTCFlagHandler(DNSResponseHandler):
    ...
    def handle(self, response_wire, response, response_time):
        ...
        if isinstance(response_wire, str):
            def map_func(x): return ord(x)
        else:
            def map_func(x): return x
...
class ChangeEDNSVersionOnTimeoutHandler(DNSResponseHandler):
    ...
    def handle(self, response_wire, response, response_time):
        ...
        if not self._params['tcp'] and timeouts >= self._timeouts and self._request.edns != self._edns:
            self._request.use_edns(self._edns, self._request.ednsflags,
                                   self._request.payload, options=self._request.options)
...
class RemoveEDNSOptionOnTimeoutHandler(DNSResponseHandler):
    ...
    def handle(self, response_wire, response, response_time):
        timeouts = self._get_num_timeouts(response)
        filtered_options = [
            x for x in self._request.options if self._otype == x.otype]
...
class PMTUBoundingHandler(DNSResponseHandler):
    ...
    def handle(self, response_wire, response, response_time):
        ...
        is_valid = isinstance(response, dns.message.Message) and response.rcode() in (
            dns.rcode.NOERROR, dns.rcode.NXDOMAIN)
        ...
        if self._state == self.INVALID:
            ...
            if is_timeout:
                self._upper_bound = self._request.payload - 1
                payload = self._lower_bound + \
                    (self._upper_bound + 1 - self._lower_bound)//2
                self._request.payload = payload
        ...
        try:
            for handler in self._response_handlers:
                if retry_action is None:
                    retry_action = handler.handle(
                        response_wire, response, response_time)
...
class AggregateDNSResponse(object):
        ...
        if response.is_valid_response():
            if response.is_complete_response():
                ...
                self._aggregate_answer(
                    server, client, response, is_referral, qname, rdtype)
            else:
                truncated_info = TruncatedResponse(response.message.to_wire())
                DNSResponseComponent.insert_into_list(
                    truncated_info, self.truncated_info, server, client, response)
    ...
    def _aggregate_answer(self, server, client, response, referral, qname, rdtype):
        ...
        dname_rrsets = [
            x for x in msg.answer if x.rdtype == dns.rdatatype.DNAME]
        ...
        try:
            ...
            while i < MAX_CNAME_REDIRECTION:
                ...
                for dname_rrset in dname_rrsets:
                    if qname_sought.parent().is_subdomain(dname_rrset.name):
                        synthesized_cname_info = RRsetInfo(cname_from_dname(
                            qname_sought, dname_rrset), self.ttl_cmp, RRsetInfo(dname_rrset, self.ttl_cmp))
                        break

                try:
                    rrset_info = self._aggregate_answer_rrset(
                        server, client, response, qname_sought, rdtype, referral)
                    ...
                    if rrset_info.rrset.rdtype == dns.rdatatype.CNAME and synthesized_cname_info is not None:
                        synthesized_cname_info = rrset_info.create_or_update_cname_from_dname_info(
                            synthesized_cname_info, server, client, response)
                        synthesized_cname_info.update_rrsig_info(
                            server, client, response, msg.answer, referral)

                except KeyError:
                    if synthesized_cname_info is None:
                        raise
                    synthesized_cname_info = DNSResponseComponent.insert_into_list(
                        synthesized_cname_info, self.answer_info, server, client, response)
                    synthesized_cname_info.dname_info.update_rrsig_info(
                        server, client, response, msg.answer, referral)
            ...
            if referral and rdtype != dns.rdatatype.DS:
                # add referrals
                try:
                    rrset = [x for x in msg.authority if qname.is_subdomain(
                        x.name) and x.rdtype == dns.rdatatype.NS][0]
                except IndexError:
                    pass
                else:
                    referral_info = ReferralResponse(rrset.name)
                    DNSResponseComponent.insert_into_list(
                        referral_info, self.referral_info, server, client, response)
            ...
            neg_response_info = NegativeResponseInfo(
                qname_sought, rdtype, self.ttl_cmp)
            neg_response_info = DNSResponseComponent.insert_into_list(
                neg_response_info, neg_response_info_list, server, client, response)
            neg_response_info.create_or_update_nsec_info(
                server, client, response, referral)
            neg_response_info.create_or_update_soa_info(
                server, client, response, referral)
    ...
    def _aggregate_answer_rrset(self, server, client, response, qname, rdtype, referral):
        ...
        try:
            rrset = msg.find_rrset(
                msg.answer, qname, dns.rdataclass.IN, rdtype)
        except KeyError:
            rrset = msg.find_rrset(
                msg.answer, qname, dns.rdataclass.IN, dns.rdatatype.CNAME)

        rrset_info = RRsetInfo(rrset, self.ttl_cmp)
        rrset_info = DNSResponseComponent.insert_into_list(
            rrset_info, self.answer_info, server, client, response)

        rrset_info.update_rrsig_info(
            server, client, response, msg.answer, referral)
    ...
    def _aggregate_error(self, server, client, response):
        ...
        if msg is None:
            ...
            error_info = DNSResponseError(
                RESPONSE_ERROR_INVALID_RCODE, msg.rcode())
        error_info = DNSResponseComponent.insert_into_list(
            error_info, self.error_info, server, client, response)
...
class DNSQuery(object):
        ...
        if with_responses:
            for server in self.responses:
                ...
                for client, response in self.responses[server].items():
                    ...
                    clone.add_response(
                        server, client, response_clone, bailiwick)
    ...
    def join(self, query, bailiwick_map, default_bailiwick):
        if not (isinstance(query, DNSQuery)):
            raise ValueError(
                'A DNSQuery instance can only be joined with another DNSQuery instance.')
        ...
        if not (self.qname == query.qname and self.rdtype == query.rdtype and
                ...
                self.tcp == query.tcp):
            raise ValueError(
                'DNS query parameters for DNSQuery instances being joined must be the same.')
    ...
    def project(self, servers, bailiwick_map, default_bailiwick):
        if servers.difference(self.responses):
            raise ValueError(
                'A DNSQuery can only project responses from servers that have been queried.')
    ...
    def add_response(self, server, client, response, bailiwick):
        ...
        if response.query is not None and response.query is not self:
            raise ValueError('Response for %s/%s is already associated with a query.' %
                             (self.qname, dns.rdatatype.to_text(self.rdtype)))
        if client in self.responses[server]:
            raise ValueError('Response for %s/%s from server %s to client %s already exists.' %
                             (self.qname, dns.rdatatype.to_text(self.rdtype), server, client))
        response.query = self
    ...
    def serialize(self, meta_only=False):
        ...
        if self.edns >= 0:
            ...
            for o in self.edns_options:
                s = io.BytesIO()
                o.to_wire(s)
                d['options']['edns_options'].append(
                    (o.otype, lb2s(base64.b64encode(s.getvalue()))))
    ...
    def deserialize(self, d, bailiwick_map, default_bailiwick):
        ...
        if 'edns_version' in d1:
            ...
            for otype, data in d1['edns_options']:
                edns_options.append(dns.edns.GenericOption(
                    otype, base64.b64decode(data)))
        else:
            edns = None
        ...
        for server in d['responses']:
            ...
            for client in d['responses'][server]:
                q.add_response(IPAddr(server), IPAddr(client), DNSResponse.deserialize(
                    d['responses'][server][client], q), bailiwick)
        return q
...
class DNSQueryAggregateDNSResponse(DNSQuery, AggregateDNSResponse):
    ...
    def add_response(self, server, client, response, bailiwick):
        super(DNSQueryAggregateDNSResponse, self).add_response(
            server, client, response, bailiwick)
        self._aggregate_response(
            server, client, response, self.qname, self.rdtype, bailiwick)
...
class MultiQuery(object):
    ...
    def add_query(self, query, bailiwick_map, default_bailiwick):
        if not (self.qname == query.qname and self.rdtype == query.rdtype and self.rdclass == query.rdclass):
            raise ValueError(
                'DNS query information must be the same as that to which query is being joined.')
        ...
        params = (query.flags, query.edns, query.edns_max_udp_payload,
                  query.edns_flags, edns_options_str, query.tcp)
        if params in self.queries:
            self.queries[params] = self.queries[params].join(
                query, bailiwick_map, default_bailiwick)
        else:
    ...
    def project(self, servers, bailiwick_map, default_bailiwick):
        ...
        for params in self.queries:
            query.add_query(self.queries[params].project(
                servers, bailiwick_map, default_bailiwick))
        return query
...
class MultiQueryAggregateDNSResponse(MultiQuery, AggregateDNSResponse):
    ...
    def add_query(self, query, bailiwick_map, default_bailiwick):
        super(MultiQueryAggregateDNSResponse, self).add_query(
            query, bailiwick_map, default_bailiwick)
        for server in query.responses:
            bailiwick = bailiwick_map.get(server, default_bailiwick)
            for client, response in query.responses[server].items():
...
class ExecutableDNSQuery(DNSQuery):
    ...
    def __init__(self, qname, rdtype, rdclass, servers, bailiwick,
        ...
        if not servers:
            raise ValueError(
                "At least one server must be specified for an ExecutableDNSQuery")
        ...
        if lifetime is None and max_attempts is None:
            raise ValueError(
                "At least one of lifetime or max_attempts must be specified for an ExecutableDNSQuery instance.")
    ...
    def get_query_handler(self, server):
        ...
        request.find_rrset(request.question, self.qname, self.rdclass,
                           self.rdtype, create=True, force_unique=True)
        request.use_edns(self.edns, self.edns_flags,
                         self.edns_max_udp_payload, options=self.edns_options[:])
        ...
        params = {'tcp': self.tcp, 'sport': None,
                  'wait': 0, 'timeout': self.query_timeout}
        ...
        if self.max_attempts is not None:
            response_handlers.append(
                MaxTimeoutsHandler(self.max_attempts).build())
    ...
    def execute_queries(cls, *queries, **kwargs):
        ...
        while query_handlers:
            ...
            for qtm in th.qtms:
                ...
                response = qh.handle_response(
                    qtm.res, response, response_time, qtm.src, qtm.sport)
                ...
                if isinstance(response, dns.message.Message):
                    ...
                    if hasattr(response, 'errno'):
                        ...
                        errno1 = None
                response_obj = DNSResponse(
                    msg, msg_size, err, errno1, qh.history, response_time, query)
                ...
                if err == RESPONSE_ERROR_NETWORK_ERROR:
                    if errno1 == errno.EADDRNOTAVAIL:
                        ...
                        if qh._client is not None:
                            raise SourceAddressBindError('Unable to bind to local address %s (%s)' % (
                                qh._client, errno.errorcode[errno1]))
                        else:
                            raise SourceAddressBindError(
                                'Unable to bind to local address (%s)' % (errno.errorcode[errno1]))
                        ...
                        if qh.params['sport'] is not None:
                            raise PortBindError('Unable to bind to local port %d (%s)' % (
                                qh.params['sport'], errno.errorcode[errno1]))
                        else:
                            raise PortBindError(
                                'Unable to bind to local port (%s)' % (errno.errorcode[errno1]))
                        ...
                        # raise an error.
                        raise BindError('Unable to bind to local address (%s)' % (
                            errno.errorcode.get(errno1, "unknown")))

                # if src is None, then it is a connectivity issue on our
                # side, so don't record it in the responses
                if src is not None:
                    query.add_response(
                        qh._server, src, response_obj, query.bailiwick)
    ...
    def add_response(self, server, client, response, bailiwick):
        super(ExecutableDNSQuery, self).add_response(
            server, client, response, bailiwick)
    ...
    def execute(self, ignore_queryid=True, tm=None, th_factories=None):
        self.execute_queries(self, ignore_queryid=ignore_queryid,
                             tm=tm, th_factories=th_factories)

    join = require_executed(DNSQuery.join)
    project = require_executed(DNSQuery.project)
    is_authoritative_answer_all = require_executed(
        DNSQuery.is_authoritative_answer_all)
...
class StandardRecursiveQueryCD(StandardRecursiveQuery):
    ...
    response_handlers = StandardRecursiveQuery.response_handlers + \
...
class DiagnosticQuery(DNSSECQuery):
    ...
    response_handlers = DNSSECQuery.response_handlers + \
        [DisableEDNSOnFormerrHandler(), DisableEDNSOnRcodeHandler(),
         ...
         ClearEDNSFlagOnTimeoutHandler(
             dns.flags.DO, 6), DisableEDNSOnTimeoutHandler(7),
...
class RecursiveDiagnosticQuery(RecursiveDNSSECQuery):
    ...
    response_handlers = DNSSECQuery.response_handlers + \
        [DisableEDNSOnFormerrHandler(), SetFlagOnRcodeHandler(dns.flags.CD, dns.rcode.SERVFAIL), DisableEDNSOnRcodeHandler(),
         ...
         ClearEDNSFlagOnTimeoutHandler(
             dns.flags.DO, 7), DisableEDNSOnTimeoutHandler(8),
         ChangeTimeoutOnTimeoutHandler(2.0, 2),
         ChangeTimeoutOnTimeoutHandler(4.0, 3),
...
class RecursiveTCPDiagnosticQuery(RecursiveDNSSECQuery):
    ...
    response_handlers = [
        DisableEDNSOnFormerrHandler(), SetFlagOnRcodeHandler(
            dns.flags.CD, dns.rcode.SERVFAIL), DisableEDNSOnRcodeHandler(),
        ChangeTimeoutOnTimeoutHandler(4.0, 2),
...
class TruncationDiagnosticQuery(DNSSECQuery):
    ...
    response_handlers = [ChangeTimeoutOnTimeoutHandler(
        2.0, 2), ChangeTimeoutOnTimeoutHandler(4.0, 3)]
    # For timeouts:
    #  1 - no change
    #  2 - change timeout to 2 seconds
    #  3 - change timeout to 4 seconds
...
def main():
    ...
    def usage():
        sys.stderr.write(
            'Usage: %s [-r] [-j] <name> <type> <server> [<server>...]\n' % (sys.argv[0]))
        sys.exit(1)
    ...
    d = cls(dns.name.from_text(args[0]), dns.rdatatype.from_text(
        args[1]), dns.rdataclass.IN, [IPAddr(x) for x in args[2:]])
    d.execute()
